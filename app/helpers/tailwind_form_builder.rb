class TailwindFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def tw_submit(value = nil, options = {})
    default_class = "transform rounded-lg bg-blue-500 px-6 py-2 text-sm font-medium tracking-wide text-white transition-colors duration-300 hover:bg-blue-400 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-50"
  ...
  private
    ...
    def input_class(field)
      "#{error_input_class(field)} mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500"
    end
    ...
    def hint_html(options)
      return unless options[:hint].present?
